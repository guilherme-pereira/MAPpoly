...
test_that("Data loaded correctly when using 'read_geno_csv'", {
  ## Tetraploid
  solcap.file <- system.file('extdata', 'tetra_solcap.csv', package = 'mappoly')
  tetra.solcap.new <- read_geno_csv(file.in  = solcap.file, ploidy = 4)
  expect_equal(check_data_sanity(tetra.solcap.new), 0)
  expect_equivalent(as.numeric(as.matrix(tetra.solcap.new$geno.dose))[1:20], 
                    c(4, 5, 3, 2, 3, 2, 5, 5, 1, 3, 
                      1, 4, 2, 1, 3, 2, 0, 3, 2, 2))
...
expect_equivalent(as.numeric(as.matrix(dat.dose.vcf$geno.dose))[1:20], 
                    c(0, 3, 3, 3, 0, 0, 0, 4, 1, 1, 
                      0, 1, 0, 0, 2, 0, 0, 0, 2, 1))
dat.dose.vcf = read_vcf(file = tempfl, parent.1 = "PARENT1", parent.2 = "PARENT2", elim.redundant = FALSE)
expect_equal(check_data_sanity(dat.dose.vcf), 0)
expect_equivalent(as.numeric(as.matrix(dat.dose.vcf$geno.dose))[1:20], 
                  c(0, 0, 3, 3, 3, 0, 0, 0, 0, 4, 
                    1, 1, 1, 0, 0, 1, 0, 0, 0, 2))
dat.dose.vcf = read_vcf(file = tempfl, parent.1 = "PARENT1", parent.2 = "PARENT2", filter.non.conforming = FALSE)
expect_equal(check_data_sanity(dat.dose.vcf), 0)
expect_equivalent(as.numeric(as.matrix(dat.dose.vcf$geno.dose))[1:20], 
                  c(0, 3, 3, 3, 0, 0, 0, 4, 1, 1, 
                    0, 1, 0, 0, 2, 0, 0, 0, 2, 1))
